...
dat <- read.csv('regions.csv')
head(dat)
dim(dat)
length(unique(dat$REGION))
range(dat$YEAR)

dat <- dat[dat$YEAR < 2016 & dat$YEAR > 1969,]

dat$ratio <- dat$ATTACKS/dat$POP1
...
# Sum attacks and population size for all years within a region
dat2 <- aggregate(cbind(dat$ATTACKS,dat$POP1), by=list(dat$YEAR,dat$REGION),FUN=sum)
head(dat2)
dim(dat2)

colnames(dat2) <- c('YEAR','REGION','ATTACKS','POP')
head(dat2)
dim(dat2)
...
dat$region2 <- as.numeric(dat$REGION)
...
# Save BUGS output and data
saveRDS(out, file="regions.bugs.rds")
saveRDS(dat, file="regions.dat.rds")
